...
if (-not $EhWindows) {
    ...
    Write-Host "ExecuÁ„o abortada para prevenir falhas de diretÛrio ou comandos." -ForegroundColor Red
    Write-Host "======================================================================`n`n" -ForegroundColor Red
    exit 1
}
    ...
    if ($BancosParaBackup.Count -eq 0) {
        ...
        foreach ($Banco in $BancosParaBackup) {
            ...
            if (Test-Path $CaminhoSql) {
                ...
                #Wait-ProcessWithSpinner -Process $ProcessoRarInd -Mensagem "Compacta√ß√£o do Banco de Dados (Dump) individual em andamento... AGUARDE..."
                Start-Sleep -Seconds 3
                if ($ProcessoRarInd.ExitCode -eq 0) {
                    ...
                    Write-Host "[ERRO] Falha ao compactar o banco $Banco. (CÛdigo do Erro: $($ProcessoRarInd.ExitCode))" -ForegroundColor Red
                    exit 1
                }
            }
        }
...
if (Test-Path $CaminhoCompletoRar) {
    ...
    Write-Host "Limpeza: Artefatos na mem√≥ria e chaves removidos com sucesso...`n" -ForegroundColor DarkCyan
    Start-Sleep -Seconds 1
